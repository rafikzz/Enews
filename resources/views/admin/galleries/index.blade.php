    ...
    <div class="row card card-outline card-dark">
        <div class="card-header">
            ...
            <div class="card-tools">
                <a class="btn btn-success" href="{{ route('admin.galleries.create') }}"> <i class="fa fa-plus"></i></a>
            </div>
        </div>
        <div class="card-body">
                ...
                <tbody id="tablecontents">
                    @foreach ($galleries as $key => $gallery)
                        <tr class="row1" data-id="{{ $gallery->id }}">
                            ...
                            <td>
                                <button
                                    class="changeStatus btn btn-sm {{ $gallery->status ? 'btn-success' : 'btn-danger' }}"
                                    data-value="{{ $gallery->status }}"
                                    data-id="{{ $gallery->id }}">{{ $gallery->status ? 'Active' : 'Inactive' }}</button>
                            </td>
                            ...
                            <td>
                                <form action="{{ route('admin.galleries.destroy', $gallery->id) }}"
                                    ...
                                    <a class="btn btn-sm btn-info"
                                        href="{{ route('admin.galleries.show', $gallery->id) }}"><i
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.btn-delete', function(e) {
                ...
                Swal.fire({
                    title: 'Are you sure?',
                    text: "Do you want to  delete gallery?",
                    icon: 'danger',
            ...
            function sendOrderToServer() {
                ...
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('admin.gallery.updateOrder') }}",
                    data: {
                        order: order,
                        "_token": "{{ csrf_token() }}",
                    },
        ...
        $(document).on('click', '.changeStatus', function() {
            ...
            Swal.fire({
                title: 'Are you sure?',
                text: "Do you want to change gallery status?",
                icon: 'warning',
                showCancelButton: true,
                confirmButtonColor: '#3085d6',
                cancelButtonColor: '#d33',
                ...
                if (result.isConfirmed) {
                    let gallery_id = btn.data('id');
                    let status = ($(this).data('value') == 0) ? 1 : 0;
                    let clickedBtn = btn;
                    $.ajax({
                        type: "GET",
                        url: '{{ route('galleries.changeStatus') }}',
                        data: {
                            'status': status,
